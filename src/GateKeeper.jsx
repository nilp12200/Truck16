...
function GateKeeper() {
  ...
  const convertToIST = (date) => {
    ...
    return new Date(localDate.toLocaleString('en-IN', { timeZone: 'Asia/Kolkata' }));
  };
  ...
  const handleSubmit = async (type) => {
    ...
    // Convert dispatch date to IST
    const dispatchDateIST = convertToIST(dispatchDate);
    ...
    try {
      const response = await axios.post(`${API_URL}/api/update-truck-status`, {
        ...
        plantName: selectedPlant,
        type,
        dispatchDate: dispatchDateIST.toISOString(),  // Sending IST dispatch date
        invoicenumber: type === 'Check Out' ? invoiceNo : '',  
        quantity: quantityPanels.reduce((acc, p) => acc + (p.quantity || 0), 0),
      });
