    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-100 p-4">
      <div className="bg-white shadow-xl rounded-lg p-0 w-full max-w-3xl relative border border-gray-300">
        ...
        {!loading && !error && (
          <div className="overflow-x-auto max-h-[70vh]">
            <table className="w-full text-sm text-left border border-gray-400 mt-0">
              <thead className="bg-cyan-700 text-white text-base sticky top-0 z-10">
                <tr>
                  <th className="px-3 py-2 border border-gray-400">TRUCK NO</th>
                  <th className="px-3 py-2 border border-gray-400">TRANSACTION DATE</th>
                  <th className="px-3 py-2 border border-gray-400">CITY NAME</th>
              ...
              <tbody>
                {truckData.length === 0 ? (
                  ...
                  truckData.map((truck, idx) => (
                    <tr key={idx} className={idx % 2 === 0 ? 'bg-white' : 'bg-cyan-50'}>
                      <td className="px-3 py-2 border border-gray-300 text-blue-800 font-semibold">
                        {truck.truckno || '—'}
                      </td>
                      <td className="px-3 py-2 border border-gray-300 font-medium">
                        {truck.transactiondate
                          ? new Date(truck.transactiondate).toLocaleDateString()
                          : '—'}
                      </td>
                      <td className="px-3 py-2 border border-gray-300 text-green-700 font-semibold">
                        {truck.cityname || '—'}
                      </td>
                      <td className="px-3 py-2 border border-gray-300 text-center">
                        <button
                          className="bg-yellow-400 hover:bg-yellow-500 text-white font-bold py-1 px-3 rounded transition-all duration-200"
                          onClick={() => navigate('/truck-transaction', { state: { truck } })}
                        >
                          Edit
                        </button>
                      </td>
