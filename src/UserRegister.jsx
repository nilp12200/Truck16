...
// API URL from environment variables (Vite setup)
const API_URL = import.meta.env.VITE_API_URL;
      ...
      {loading ? (
        ...
        <div style={{ overflowX: 'auto', borderRadius: 16, background: '#f4f6fa', boxShadow: '0 2px 12px rgba(0,0,0,0.07)' }}>
          <table style={{ width: '100%', borderCollapse: 'separate', borderSpacing: 0, minWidth: 600 }}>
            <thead>
              <tr style={{ background: '#1976d2', color: '#fff' }}>
                <th style={{ padding: '14px', border: 'none', fontWeight: 700, letterSpacing: 1 }}>Username</th>
                <th style={{ padding: '14px', border: 'none', fontWeight: 700, letterSpacing: 1 }}>Role</th>
                <th style={{ padding: '14px', border: 'none', fontWeight: 700, letterSpacing: 1 }}>Contact Number</th>
            ...
            <tbody>
              {users.length === 0 ? (
                <tr>
                  <td colSpan="4" style={{ textAlign: 'center', padding: '1.5rem', color: '#888' }}>No users found.</td>
                </tr>
              ) : (
                users.map((user, idx) => (
                  <tr key={idx} style={{ background: idx % 2 === 0 ? '#fff' : '#e3eafc', transition: 'background 0.2s' }}>
                    <td style={{ padding: '12px', fontWeight: 500 }}>{user.username}</td>
                    <td style={{ padding: '12px', fontWeight: 500 }}>{user.role}</td>
                    <td style={{ padding: '12px', fontWeight: 500 }}>{user.contactnumber}</td>
                    <td style={{ padding: '12px', fontWeight: 500 }}>
                      {user.allowedPlants.length > 0 ? user.allowedPlants.join(', ') : '-'}
                    </td>
                  </tr>
                ))
              )}
