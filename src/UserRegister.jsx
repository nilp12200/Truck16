  ...
  const fetchAllData = async () => {
    ...
    try {
      const [userRes, plantRes] = await Promise.all([
        ...
        fetch(`${API_URL}/api/plant-master`)
      ]);
      ...
      {loading ? (
        ...
        <div style={{
          ...
          <table style={{ width: '100%', borderCollapse: 'separate', borderSpacing: 0, minWidth: 900 }}>
            <thead>
              <tr style={{ background: '#1976d2', color: '#fff' }}>
                <th style={{ padding: '14px', fontWeight: 700 }}>Username</th>
                <th style={{ padding: '14px', fontWeight: 700 }}>Password</th>
                <th style={{ padding: '14px', fontWeight: 700 }}>Role</th>
                <th style={{ padding: '14px', fontWeight: 700 }}>Allowed Plants</th>
              </tr>
            </thead>
            <tbody>
              {users.length === 0 ? (
                ...
                <tr key={idx} style={{ background: idx % 2 === 0 ? '#fff' : '#e3eafc' }}>
                  ...
                  <td style={{ padding: '12px', fontWeight: 500 }}>
                    {getPlantNamesFromIds(user.allowed_plant || '')}
                  </td>
                </tr>
              ))}
            </tbody>
