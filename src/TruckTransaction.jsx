...
export default function TruckTransaction() {
  ...
  const checkFormValidity = () => {
    const requiredFieldsFilled = formData.truckNo && formData.transactionDate && formData.cityName &&
      tableData.every(row => row.plantName && row.loadingSlipNo && row.qty && row.priority && row.freight);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-100 to-gray-50 py-8">
      ...
      <div className="max-w-4xl mx-auto bg-white rounded-2xl shadow-xl p-6 md:p-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          <div>
            <label className="font-medium text-slate-700 mb-1 block">Truck No <span className="text-red-600">*</span></label>
            <input type="text" name="truckNo" maxLength={11} value={formData.truckNo} onChange={handleChange}
              placeholder="Enter truck number"
              className="w-full p-3 border border-slate-300 rounded-lg bg-slate-50 focus:outline-none focus:ring-2 focus:ring-indigo-400" />
          </div>
          {['transactionDate', 'cityName', 'transporter'].map((field) => (
            <div key={field}>
              <label className="font-medium text-slate-700 mb-1 block">{field.replace(/([A-Z])/g, ' $1').toUpperCase()}</label>
        ...
        <table className="min-w-full bg-white border border-slate-300">
          <thead>
            <tr>
              <th className="text-left p-2">Plant <span className="text-red-600">*</span></th>
              <th className="text-left p-2">Loading Slip No <span className="text-red-600">*</span></th>
              <th className="text-left p-2">Qty <span className="text-red-600">*</span></th>
              <th className="text-left p-2">Priority <span className="text-red-600">*</span></th>
          ...
          <tbody>
            {tableData.map((row, idx) => (
              <tr key={idx}>
                <td>
                  <input type="text" name="plantName" value={row.plantName} onChange={(e) => handleRowChange(idx, e)}
                    className="w-full p-3 border border-slate-300 rounded-lg bg-slate-50 focus:outline-none focus:ring-2 focus:ring-indigo-400" />
                </td>
                <td>
                  <input type="text" name="loadingSlipNo" value={row.loadingSlipNo} onChange={(e) => handleRowChange(idx, e)}
                    className="w-full p-3 border border-slate-300 rounded-lg bg-slate-50 focus:outline-none focus:ring-2 focus:ring-indigo-400" />
                </td>
                <td>
                  <input type="number" name="qty" value={row.qty} onChange={(e) => handleRowChange(idx, e)}
                    className="w-full p-3 border border-slate-300 rounded-lg bg-slate-50 focus:outline-none focus:ring-2 focus:ring-indigo-400" />
                </td>
                <td>
                  <input type="text" name="priority" value={row.priority} onChange={(e) => handleRowChange(idx, e)}
                    className="w-full p-3 border border-slate-300 rounded-lg bg-slate-50 focus:outline-none focus:ring-2 focus:ring-indigo-400" />
                </td>
                <td>
                  <input type="text" name="remarks" value={row.remarks} onChange={(e) => handleRowChange(idx, e)}
                    className="w-full p-3 border border-slate-300 rounded-lg bg-slate-50 focus:outline-none focus:ring-2 focus:ring-indigo-400" />
                </td>
                <td>
                  <input type="text" name="freight" value={row.freight} onChange={(e) => handleRowChange(idx, e)}
                    className="w-full p-3 border border-slate-300 rounded-lg bg-slate-50 focus:outline-none focus:ring-2 focus:ring-indigo-400" />
                </td>
                <td>
                  <button className="p-2 bg-red-500 text-white rounded-lg" onClick={() => handleDeleteRow(idx)}>Delete</button>
                </td>
              </tr>
            ))}
          </tbody>
        ...
        <div className="mt-6">
          <button onClick={addOrUpdateRow} className="py-2 px-4 bg-indigo-500 text-white rounded-lg">Add Row</button>
        </div>

        <div className="mt-6">
          <button onClick={handleSubmit} className="py-2 px-6 bg-blue-500 text-white rounded-lg" disabled={!isFormValid}>
            Submit
          </button>
        </div>

        {message && <div className="mt-6 text-center text-lg text-red-500">{message}</div>}
      </div>
    </div>
  );
}
