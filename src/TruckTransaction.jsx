...
export default function TruckTransaction() {
  ...
  const fetchTruckDetails = async (truckNo) => {
    try {
      ...
      if (err.response?.status === 409) setMessage('🚫 Truck is already in transport. Please complete Check-Out first.');
      else if (err.response?.status === 404) setMessage('Truck not found. You can create a new transaction.');
      else {
        console.error('Error loading truck details:', err);
        setMessage('❌ Failed to load truck details.');
      }
  ...
  const handleChange = (e) => {
    ...
    try {
      if (name === 'truckNo') {
        value = value.replace(/[^a-zA-Z0-9]/g, '').slice(0, 11).toUpperCase();
        setFormData(f => ({ ...f, truckNo: value }));
      } else if (name === 'priority' || name === 'qty') {
        value = value.replace(/[^0-9]/g, '');
        if (name === 'priority' && editingIndex !== null) {
          setTableData(prev => {
            const arr = [...prev];
            arr[editingIndex][name] = value;
            return arr;
          });
        } else if (name === 'qty' && editingIndex !== null) {
          setTableData(prev => {
            const arr = [...prev];
            arr[editingIndex][name] = value;
            return arr;
      ...
      setMessage('❌ An error occurred while processing the input.');
    }
  };
  ...
  const handleEditRow = idx => setEditingIndex(idx);
  const handleUpdateRow = idx => {
    const newPriority = tableData[idx].priority;
    if (tableData.some((r, i) => i !== idx && r.priority === newPriority)) {
      setMessage(`❌ Priority ${newPriority} already exists.`);
      return;
    }
    setEditingIndex(null);
  };
  ...
  const addOrUpdateRow = () => {
    ...
    if (tableData.some(r => r.plantName === newRow.plantName)) {
      setMessage(`❌ Plant ${newRow.plantName} already selected.`);
      return;
    }
    if (tableData.some(r => r.priority === newRow.priority)) {
      setMessage(`❌ Priority ${newRow.priority} already exists.`);
      return;
    }
    setTableData(prev => [...prev, { ...newRow, detailId: null }]);
    setNewRow({ detailId: null, plantName: '', loadingSlipNo: '', qty: '', priority: '', remarks: '', freight: 'To Pay' });
    setMessage('');
  };

  const validateForm = () => {
    const requiredFormFields = ['truckNo', 'transactionDate', 'cityName', 'deliverPoint', 'truckWeight'];
    ...
    for (const [i, row] of tableData.entries()) {
      if (!row.plantName || !row.loadingSlipNo || !row.qty || !row.priority) {
        setMessage(`❌ Please fill all required fields in table row ${i + 1}.`);
        return false;
      }
    }
    return true;
  ...
  const handleSubmit = async () => {
    ...
    const pending = newRow.plantName || newRow.loadingSlipNo || newRow.qty || newRow.priority;
    if (pending) {
      if (!newRow.plantName || !newRow.loadingSlipNo || !newRow.qty || !newRow.priority) {
        setMessage('❌ Complete all fields in new row before submitting.');
        return;
      }
      dataToSubmit.push({ ...newRow, detailId: null });
    }
    ...
    try {
      const res = await axios.post(`${API_URL}/api/truck-transaction`, { formData, tableData: dataToSubmit });
      if (res.data.success) {
        setMessage('✅ Transaction saved!');
        setFormData({ transactionId: null, truckNo: '', transactionDate: '', cityName: '', transporter: '', amountPerTon: '', truckWeight: '', deliverPoint: '', remarks: '' });
        setTableData([]);
        setNewRow({ detailId: null, plantName: '', loadingSlipNo: '', qty: '', priority: '', remarks: '', freight: 'To Pay' });
      } else setMessage('❌ Error saving transaction.');
    } catch (err) {
      if (err.response?.status === 409) {
        setMessage('🚫 Truck in transport. Complete Check-Out first.');
      } else {
  ...
  const selectedPlants = tableData.map((r, i) => i === editingIndex ? null : r.plantName);
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-100 to-gray-50 py-8">
      ...
      <div className="max-w-4xl mx-auto bg-white rounded-2xl shadow-xl p-6 md:p-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          {/* Truck No */}
          <div>
            <label className="font-medium text-slate-700 mb-1 block">Truck No {requiredStar}</label>
            <input name="truckNo" value={formData.truckNo} onChange={handleChange} placeholder="Enter Truck No" className="w-full p-3 border rounded-lg" maxLength={11}/>
          </div>
          {/* Transaction Date, City Name, Transporter */}
          {[
            { field: 'transactionDate', label: 'Transaction Date', type: 'date', required: true },
            { field: 'cityName', label: 'City Name', type: 'text', required: true },
            { field: 'transporter', label: 'Transporter', type: 'text', required: false }
            ...
            <div key={field}>
              ...
              <input type={type} name={field} value={formData[field]} onChange={handleChange} className="w-full p-3 border rounded-lg" />
            </div>
          ))}
        </div>
        ...
        <div className="overflow-x-auto mb-6">
          <table className="w-full text-sm text-left border shadow rounded-xl">
            <thead className="bg-indigo-500 text-white">
              <tr>
                <th>Plant{requiredStar}</th><th>Slip No{requiredStar}</th><th>Qty{requiredStar}</th><th>Priority{requiredStar}</th><th>Remarks</th><th>Freight</th><th>Actions</th>
              </tr>
            </thead>
            <tbody>
              {tableData.map((r, i) => (
                <tr key={i} className={i % 2 ? 'bg-slate-50' : 'bg-white'}>
                  {editingIndex === i ? (
                    <>
                      <td><select name="plantName" value={r.plantName} onChange={e => handleRowChange(i, e)}>
                        <option>Select</option>
                        {plantList.filter(p => !selectedPlants.includes(p.plantname) || p.plantname === r.plantName)
                          .map((p, idx) => <option key={idx} value={p.plantname}>{p.plantname}</option>)}
                      </select></td>
                      {['loadingSlipNo','qty','priority','remarks'].map(name => (
                        <td key={name}>
                          <input name={name} value={r[name]} onChange={e => handleRowChange(i, e)} className="w-full p-2 border rounded-lg" />
                        </td>
                      ))}
                      <td><select name="freight" value={r.freight} onChange={e => handleRowChange(i, e)}>
                        <option>To Pay</option><option>Paid</option>
                      </select></td>
                      <td><button onClick={() => handleUpdateRow(i)} className="bg-green-400 px-3">Update</button></td>
                    </>
                  ) : (
                    <>
                      <td>{r.plantName}</td><td>{r.loadingSlipNo}</td><td>{r.qty}</td><td>{r.priority}</td><td>{r.remarks}</td><td>{r.freight}</td>
                      <td>
                        <button onClick={() => handleEditRow(i)} className="bg-yellow-300 px-3">Edit</button>
                        <button onClick={() => handleDeleteRow(i)} className="bg-red-300 px-3">Delete</button>
                      </td>
                    </>
                  )}
                </tr>
              ...
              {/* New row entry */}
              <tr className="bg-slate-100">
                <td><select name="plantName" value={newRow.plantName} onChange={handleNewRowChange}>
                  <option>Select</option>
                  {plantList.filter(p => !selectedPlants.includes(p.plantname))
                    .map((p, idx) => <option key={idx} value={p.plantname}>{p.plantname}</option>)}
                </select></td>
                {['loadingSlipNo','qty','priority','remarks'].map(name => (
                  <td key={name}><input name={name} value={newRow[name]} onChange={handleNewRowChange} className="w-full p-2 border rounded-lg" /></td>
                ))}
                <td>
                  <select name="freight" value={newRow.freight} onChange={handleNewRowChange}>
                    <option>To Pay</option><option>Paid</option>
        ...
        <button onClick={addOrUpdateRow} className="bg-yellow-400 px-4 py-2 rounded-full mb-6">Add Row</button>

        {/* Bottom form fields - amountPerTon is optional */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
          {[
            { field: 'amountPerTon', label: 'Amount Per Ton', required: false },
            { field: 'deliverPoint', label: 'Deliver Point', required: true },
            ...
            <div key={field}>
              ...
              <input name={field} value={formData[field]} onChange={handleChange} className="w-full p-3 border rounded-lg" />
            </div>
          ))}
        </div>

        <label className="font-medium text-slate-700 mb-1 block">Remarks</label>
        <textarea name="remarks" value={formData.remarks} onChange={handleChange} rows="3" className="w-full p-3 border rounded-lg mb-4"></textarea>

        <button onClick={handleSubmit} className="bg-green-500 text-white px-6 py-2 rounded-full">Submit</button>
