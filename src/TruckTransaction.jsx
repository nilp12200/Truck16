...
export default function TruckTransaction() {
  ...
  const fetchTruckDetails = async (truckNo) => {
    try {
      ...
      setFormData({
        ...
        cityName: master.cityname,
        transporter: master.transporter,
        amountPerTon: master.amountperton || '0',  // Ensure amountPerTon is set to '0' if not available
        truckWeight: master.truckweight,
        deliverPoint: master.deliverpoint,
        remarks: master.remarks
      });
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await axios.post(`${API_URL}/api/truck-transaction`, {
        ...formData, details: dataToSubmit
      });
      console.log('Response:', response.data);
      // Do something after the form is successfully submitted
    } catch (error) {
      console.error('Error submitting truck transaction:', error);
    }
  };

  return (
    <div>
      <h2>Truck Transaction Form</h2>
      <form>
        {/* Input Fields */}
        <div>
          <label>Truck No {requiredStar}</label>
          <input type="text" name="truckNo" value={formData.truckNo} onChange={handleChange} maxLength={11} />
        </div>
        <div>
        ...
        <div>
          <label>Amount Per Ton {requiredStar}</label>
          <input type="number" name="amountPerTon" value={formData.amountPerTon} onChange={handleChange} />
        </div>
        <div>
          <label>Truck Weight {requiredStar}</label>
          <input type="number" name="truckWeight" value={formData.truckWeight} onChange={handleChange} />
        </div>
        <div>
      ...
      {/* Table Component (If you need to render table rows) */}
      <table>
        <thead>
          <tr>
            <th>Plant Name</th>
