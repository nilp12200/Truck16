...
function Navbar() {
  ...
  useEffect(() => {
    ...
    if (roles) {
      setUserRole(roles.split(',').map(r => r.trim().toLowerCase()));
    }
  }, []);
  ...
  const canAccess = (route) => {
    return userRole.some(role => roleAccess[role]?.includes(route));
  };
  ...
  return (
    <nav className="bg-gray-900 text-white px-4 py-3 shadow-md">
      <div className="max-w-7xl mx-auto flex justify-between items-center">
        <div className="text-xl font-bold bg-gradient-to-r from-yellow-300 to-yellow-500 text-transparent bg-clip-text">
          Lemon Gate Pass
        </div>

        {/* Desktop menu */}
        <div className="hidden md:flex space-x-6 items-center">
          {canAccess('plantmaster') && <Link to="/plantmaster" className="hover:text-yellow-400">🏭 Plant Master</Link>}
          {canAccess('usermaster') && <Link to="/usermaster" className="hover:text-yellow-400">👤 User Master</Link>}
          {canAccess('truck') && <Link to="/truck" className="hover:text-yellow-400">🚛 Truck Transaction</Link>}
          {canAccess('truckfind') && <Link to="/truckfind" className="hover:text-yellow-400">🔍 Truck Find</Link>}
          {canAccess('gate') && <Link to="/gate" className="hover:text-yellow-400">🚪 Gate Keeper</Link>}
          {canAccess('loader') && <Link to="/loader" className="hover:text-yellow-400">📦 Loader</Link>}
          {canAccess('reports') && <Link to="/reports" className="hover:text-yellow-400">📊 Reports</Link>}

          <button
            onClick={handleLogout}
            className="ml-4 px-3 py-1 bg-red-500 rounded hover:bg-red-600 text-sm font-medium"
          >
            Logout
          </button>
        </div>
      ...
      {/* Mobile menu */}
      {mobileMenuOpen && (
        <div className="md:hidden mt-3 space-y-3 bg-gray-800 p-4 rounded-lg text-white font-medium">
          {canAccess('plantmaster') && <Link to="/plantmaster" className="block hover:text-yellow-400">🏭 Plant Master</Link>}
          {canAccess('usermaster') && <Link to="/usermaster" className="block hover:text-yellow-400">👤 User Master</Link>}
          {canAccess('truck') && <Link to="/truck" className="block hover:text-yellow-400">🚛 Truck Transaction</Link>}
          {canAccess('truckfind') && <Link to="/truckfind" className="block hover:text-yellow-400">🔍 Truck Find</Link>}
          {canAccess('gate') && <Link to="/gate" className="block hover:text-yellow-400">🚪 Gate Keeper</Link>}
          {canAccess('loader') && <Link to="/loader" className="block hover:text-yellow-400">📦 Loader</Link>}
          {canAccess('reports') && <Link to="/reports" className="block hover:text-yellow-400">📊 Reports</Link>}

          <button
            onClick={handleLogout}
            className="block w-full text-left bg-red-500 hover:bg-red-600 px-3 py-2 mt-3 rounded"
          >
            Logout
          </button>
        </div>
